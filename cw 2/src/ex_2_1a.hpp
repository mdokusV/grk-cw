...
void init(GLFWwindow* window)
{
	...
	// Zainicj alizuj VertexArrayObject (VAO) i zapisz w nim atrybuty prostopadłościanu
	// I Stwórz VAO
	...
	///GLuint vPosition = glGetAttribLocation(program, "vertexPosition");
	GLuint vPosition = 0;
	glEnableVertexAttribArray(program);
	...
	///GLuint vColor = glGetAttribLocation(program, "vertexColor");
	GLuint vColor = 1;
	...
	glVertexAttribPointer(vPosition, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(float), 0);
	glVertexAttribPointer(vColor, 4, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(4 * sizeof(float)));
